...
def calculate_discrete_zeroshot(epoch_root, name2gt, llm, tokenizer, sampling_params, inter_print=True):
    ...
    # 计算 EW-based F1 score (precision-aware, penalizes over-prediction)
    ew_fscore, ew_precision, ew_recall = wheel_metric_calculation(name2gt=name2gt, openset_npz=openset_npz, inter_print=inter_print)
    return hitrate, mscore, ew_fscore
...
def main_zeroshot_scores(input_dir, debug=False, test_epochs='', inter_print=True):
    ...
    for epoch_root in sorted(glob.glob(input_dir + '/*.npz')):
        ...
        # 1. score calculation
        if disordim_flag == 'discrete':
            hitrate, mscore, ew_fscore = calculate_discrete_zeroshot(epoch_root, name2gt, llm, tokenizer, sampling_params, inter_print)
            if inter_print: print(f'hitrate: {hitrate}, mscore: {mscore}, ew_fscore: {ew_fscore}')
            whole_score1s.append(hitrate)
            whole_score2s.append(mscore)
            whole_score3s.append(ew_fscore)
    ...
    if disordim_flag == 'discrete':
        if inter_print: print (f'{dataset}: best hitrate: %.4f; best mscore: %.4f; best ew_fscore: %.4f' %(best_score1, best_score2, best_score3))
    elif disordim_flag == 'dimension':
...
def func_return_scores_one(modelname=None, dataset_candidates='affectgpt'):
    ...
    for dataset in process_datasets:
        process_root = f"output_grpo_desc_1sent/results-{dataset}/{modelname}"
        ## 计算指标
...
if __name__ == "__main__":
    ...
    ## step1：测试新模型下的结果
    # Qwen3-8B模型的inference结果在output_qwen3目录下
    for modelname in [
                    'grpo_format_control',
                    ]:
        for dataset in ["mer2023", "mer2024", "meld", "iemocapfour", "cmumosi", "cmumosei", "sims", "simsv2", "ovmerdplus"]:
            main_zeroshot_scores(f"output_grpo_desc_1sent/results-{dataset}/{modelname}")


    ## step2: 结果汇总展示 (hitrate + mscore + ew_fscore)
    for modelname in [
                    "grpo_format_control",
                    ]:
        datasets = ["mer2023", "mer2024", "meld", "iemocapfour", "cmumosi", "cmumosei", "sims", "simsv2", "ovmerdplus"]
        print_s1, print_s2, print_s3 = [], [], []
        avg_s1, avg_s2, avg_s3 = [], [], []
        for dataset in datasets:
            process_root = f"output_grpo_desc_1sent/results-{dataset}/{modelname}"
            score1, score2, score3 = main_zeroshot_scores(process_root, debug=True, test_epochs='', inter_print=False)
            print_s1.append("& %.2f" %(score1*100))
            print_s2.append("& %.2f" %(score2*100))
            print_s3.append("& %.2f" %(score3*100))
            avg_s1.append(score1)
            avg_s2.append(score2)
            avg_s3.append(score3)
        print_s1.append("& %.2f" %(np.mean(avg_s1)*100))
        print_s2.append("& %.2f" %(np.mean(avg_s2)*100))
        print_s3.append("& %.2f" %(np.mean(avg_s3)*100))
        print(modelname + " [hitrate/fscore] " + " ".join(print_s1))
        print(modelname + " [mscore/acc]     " + " ".join(print_s2))
        print(modelname + " [ew_f1/recall]   " + " ".join(print_s3))
