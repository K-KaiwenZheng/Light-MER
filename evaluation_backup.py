...
def func_return_scores_one(modelname=None, dataset_candidates='affectgpt'):
    ...
    for dataset in process_datasets:
        process_root = f"output_qwen3_0.6/results-{dataset}/{modelname}"
        ## 计算指标
...
if __name__ == "__main__":
    ...
    for modelname in [
                    'qwen3_0.6_emercoarse_highlevelfilter4_outputhybird_bestsetup_bestfusion_lz_20260211144',
                    ]:
        for dataset in ["mer2023", "mer2024", "meld", "iemocapfour", "cmumosi", "cmumosei", "sims", "simsv2", "ovmerdplus"]:
            main_zeroshot_scores(f"output_qwen3_0.6/results-{dataset}/{modelname}")
    ...
    for modelname in [
        ...
        # 注意：func_return_scores_one需要单独处理，因为它硬编码了"output/"路径
        # 这里直接计算每个数据集的分数
        print_scores = []
        avg_scores = []
        for dataset in ["mer2023", "mer2024", "meld", "iemocapfour", "cmumosi", "cmumosei", "sims", "simsv2", "ovmerdplus"]:
            process_root = f"output_qwen3_0.6/results-{dataset}/{modelname}"
            score1, score2, score3 = main_zeroshot_scores(process_root, debug=True, test_epochs='', inter_print=False)
            print_scores.append("& %.2f" %(score1*100))
            avg_scores.append(score1)
        # 添加平均值
        avg_score = np.mean(avg_scores)
        print_scores.append("& %.2f" %(avg_score*100))
        print (modelname, " ".join(print_scores))
