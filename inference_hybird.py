    ...
    for ii, ckpt_3 in enumerate(whole_ckpt3s):
        ...
        for dataset in process_datasets:
            ...
            for ii, name in enumerate(test_names):
                ...
                print (f'[{sample_elapsed:.2f}s] {response}')

                # if ii == 0: break # for debug

            dataset_elapsed = time.time() - dataset_start_time
            avg_time = np.mean(sample_times) if sample_times else 0
            print (f'\n====== Timing Summary for {dataset} ======')
            print (f'  Total samples: {len(sample_times)}')
            print (f'  Total time: {dataset_elapsed:.1f}s ({dataset_elapsed/60:.1f}min)')
            print (f'  Avg per sample: {avg_time:.2f}s')
            print (f'  Min: {min(sample_times):.2f}s, Max: {max(sample_times):.2f}s')
            print (f'==========================================\n')
