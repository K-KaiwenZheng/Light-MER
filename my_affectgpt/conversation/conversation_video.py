...
class Chat:
    def __init__(self, model, model_cfg, device='cuda:0'):
        ...
        # Debug: show how the tokenizer encodes ### in different contexts
        for label, text in [('###', '###'), ('.###', '.###'), (' ###', ' ###'),
                            ('tense.###', 'tense.###'), ('positive. ###', 'positive. ###')]:
            ids = self.tokenizer(text, add_special_tokens=False)['input_ids']
            decoded = [self.tokenizer.decode([i]) for i in ids]
            print(f"[StopToken] '{label}' -> ids={ids}, per_token={decoded}")
        print(f"[StopToken] eos={self.tokenizer.eos_token_id}, "
              f"tre={id_tre_jin}, two={id_two_jin}, one={id_one_jin}")
        ...
        # Add context-dependent ### tokenizations (Qwen3 encodes ' ###' differently from '###')
        for ctx_text in ['.###', ' ###', 'tense.###', 'positive. ###']:
            ctx_ids = self.tokenizer(ctx_text, add_special_tokens=False)['input_ids']
            for suffix_len in [1, 2]:
                if len(ctx_ids) >= suffix_len:
                    suffix = ctx_ids[-suffix_len:]
                    candidate = torch.tensor(suffix).to(self.device)
                    already = any(torch.equal(candidate, s) for s in stop_words_ids)
                    if not already:
                        stop_words_ids.append(candidate)
                        decoded = self.tokenizer.decode(suffix)
                        print(f"[StopToken] +extra stop from '{ctx_text}': {suffix} -> '{decoded}'")

        print(f"[StopToken] Total stop patterns: {len(stop_words_ids)}")
    ...
    def answer_sample(self, prompt, img_list, num_beams=1, temperature=1.0, do_sample=True,  top_p=0.9,
        ...
        full_decoded = self.tokenizer.decode(outputs[0], add_special_tokens=False)
        ...
        if getattr(self, '_return_raw', False):
            return response, full_decoded, num_generated_tokens
        return response
