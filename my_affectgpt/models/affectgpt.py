...
from my_affectgpt.models.ot_loss import OTProjector, sinkhorn_divergence
import config
...
class AffectGPT(Blip2Base):
    ...
    def __init__(
        ...
        # Qwen3 uses bfloat16 natively; float16 will overflow (max ~65504 vs bfloat16 max ~3.4e38)
        if llama_model_name in ['Qwen3', 'Qwen3_0.6B']:
            self._autocast_dtype = torch.bfloat16
            self.llama_model = AutoModelForCausalLM.from_pretrained(
                config.PATH_TO_LLM[llama_model_name],
                torch_dtype=torch.bfloat16
            )
        elif llama_model_name in ['Baichuan2']:
    ...
    # ===================================================== #
    # OT Distillation: Teacher Model + Optimal Transport
    # ===================================================== #
    def set_teacher(self, teacher_model, ot_weight=1.0, ot_epsilon=0.1,
                    ot_common_dim=256, ot_num_iters=20, ot_ramp_steps=5000,
                    kl_weight=0.0, kl_temperature=2.0):
        """
        设置教师模型用于 OT 蒸馏。

        Design decisions addressing known distillation risks:
          - teacher_proj is FROZEN (orthogonal init) inside OTProjector,
            preventing "shortcut" where teacher_proj adapts instead of student.
          - OT is computed ONLY on hidden-state level (answer tokens),
            not on frozen encoder features (which would be a bypass head).
          - Cosine cost (bounded [0,2]) with epsilon=0.1 ensures numerical stability.
          - Mask uses labels != IGNORE_INDEX to focus on answer/reasoning tokens,
            avoiding prompt-dominated OT.
        """
        # [Teacher setup implementation removed - paper under review]
        pass

    def _get_ot_weight(self, global_step):
        """Linear ramp-up for OT weight"""
        if not hasattr(self, 'ot_ramp_steps') or self.ot_ramp_steps <= 0:
            return self.ot_weight_target
        ratio = min(1.0, global_step / self.ot_ramp_steps)
        return self.ot_weight_target * ratio

    @property
    def teacher(self):
        if hasattr(self, '_teacher_container') and self._teacher_container:
            return self._teacher_container[0]
        return None

    def to(self, *args, **kwargs):
        result = super().to(*args, **kwargs)
        if self.teacher is not None:
            self._teacher_container[0] = self._teacher_container[0].to(*args, **kwargs)
        return result
    ...
    def _forward_with_hidden(self, samples):
        """
        Full forward pass returning hidden states and logits.
        Used by the teacher model during OT distillation.
        Only returns what's needed: last-layer hidden states + logits.
        """
    ...
    def forward(self, samples):
        ...
        need_hidden = self.teacher is not None
        ...
        # ========== OT Distillation Loss ==========
        # [OT distillation loss computation removed - paper under review]
        # This section computes:
        #   1. Teacher forward pass (frozen, no_grad)
        #   2. Hidden-state optimal transport alignment
        #   3. Optional KL divergence on logits
        #   4. Combined loss = CE + OT + KL
        if self.teacher is not None:
            ot_hidden_loss = torch.tensor(0.0, device=ce_loss.device)
            kl_loss = torch.tensor(0.0, device=ce_loss.device)
            total_loss = ce_loss  # placeholder
            return {"loss": total_loss, "ce_loss": ce_loss,
                    "ot_hidden_loss": ot_hidden_loss,
                    "kl_loss": kl_loss, "ot_weight": 0.0,
                    "logits": outputs.logits}

        return {"loss": ce_loss, "logits": outputs.logits}
