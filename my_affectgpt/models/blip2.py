...
class Blip2Base(BaseModel):
    ...
    def maybe_autocast(self, dtype=None):
        ...
        # if on gpu, use autocast with dtype if provided, otherwise use model's autocast dtype
        if dtype is None:
            dtype = getattr(self, '_autocast_dtype', torch.float16)
        enable_autocast = self.device != torch.device("cpu")

        if enable_autocast:
            # Support for PyTorch 2.x versions
            if hasattr(torch, 'amp') and hasattr(torch.amp, 'autocast'):
                # PyTorch 2.0+ uses torch.amp.autocast
                return torch.amp.autocast('cuda', dtype=dtype)
            elif hasattr(torch.cuda, 'amp') and hasattr(torch.cuda.amp, 'autocast'):
                # PyTorch 1.x and early 2.x use torch.cuda.amp.autocast
