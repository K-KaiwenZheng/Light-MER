        ...
        if amp:
            if self._scaler is None:
                if torch.__version__.startswith('2.4.0'):
                    ...
                    # PyTorch 2.9.x uses the same API as 2.4.0
                    self._scaler = torch.amp.GradScaler('cuda')
                else:
                    # Try the newer API by default for other versions
                    try:
                        self._scaler = torch.amp.GradScaler('cuda')
                    except:
                        self._scaler = torch.cuda.amp.GradScaler()
    ...
    def train(self):
        ...
        for cur_epoch in range(self.start_epoch+1, self.max_epoch+1):
            ...
            if len(self.valid_splits) > 0:
                ...
                # Modified: save checkpoint every 5 epochs to save disk space
                if not self.evaluate_only and (cur_epoch % 5 == 0 or cur_epoch == self.max_epoch):
